basic_mediation <- function(predictor, mediator, outcome, df){
  ...
  c_fit <- lm(eval(parse(text=outcome))~eval(parse(text=predictor)), data = df)
  a_fit <- lm(eval(parse(text=mediator))~eval(parse(text=predictor)), data = df)
  c_prime_fit <- lm(eval(parse(text=outcome))~eval(parse(text=predictor))+eval(parse(text=mediator)), data = df)
